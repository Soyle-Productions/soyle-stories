...
class CreateStoryEventUnitTest {
	...
	companion object {
		const val BLANK = ""
		const val ValidStoryEventName = "Valid Story Event Name"
	...
	fun `name is blank`()
	{
		given(providedName = BLANK)
		whenUseCaseIsExecuted()
		assertStoryEventNameCannotBeBlank(result)
	}
	...
	private fun given(providedName: String, providedRelativeStoryEventId: UUID? = null, insert: Boolean = Before, numberOfExistingStoryEvents: Int = 0, relativeStoryEventIndex: Int = 0)
	{
		...
		repeat(numberToMake) {
			val id = if (it == relativeStoryEventIndex && providedRelativeStoryEventId == ExistingId) StoryEvent.Id(ExistingId) else StoryEvent.Id()
	...
	fun assertStoryEventNameCannotBeBlank(actual: Any?)
	{
		actual as StoryEventCannotBeBlank
	}
