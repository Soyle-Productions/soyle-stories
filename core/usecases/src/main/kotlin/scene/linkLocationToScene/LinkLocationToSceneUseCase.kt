...
class LinkLocationToSceneUseCase(
    ...
    override suspend fun invoke(request: RequestModel, output: OutputPort) {
        val response = try {
            ...
            return output.failedToLinkLocationToScene(e)
        }
    ...
    private suspend fun update(scene: Scene, request: RequestModel) {
        val update = if (request.locationId != null) {
            ...
            scene.settings.fold<SceneSettingLocation, SceneUpdate<LocationRemovedFromScene>>(scene.noUpdate()) { prevUpdate, location ->
                prevUpdate.scene.withoutLocation(
                    location.id
                )
            }
        }
