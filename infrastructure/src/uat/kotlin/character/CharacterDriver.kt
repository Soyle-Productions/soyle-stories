...
object CharacterDriver : ApplicationTest() {
    ...
    fun registerIdentifiers(double: SoyleStoriesTestDouble, identifiers: List<Pair<String, Character.Id>>) {
        ProjectSteps.getProjectScope(double)?.get<WorkBench>()?.properties?.put(
            "characterIdFor",
            identifiers.toMap().toMutableMap()
        )
    }

    fun getCharacterIdentifiers(double: SoyleStoriesTestDouble): Map<String, Character.Id>? {
        return ProjectSteps.getProjectScope(double)
            ?.get<WorkBench>()?.properties?.get("characterIdFor") as? MutableMap<String, Character.Id> ?: run {
			registerIdentifiers(double, listOf())
            ProjectSteps.getProjectScope(double)
                ?.get<WorkBench>()?.properties?.get("characterIdFor") as? MutableMap<String, Character.Id>
        }
    }

    fun getCharacterIdByIdentifier(double: SoyleStoriesTestDouble, identifier: String): Character.Id? {
        return getCharacterIdentifiers(double)?.get(identifier) ?: run {
            val characterWithName = getCharactersCreated(double).find { it.name == identifier }?.also {
                getCharacterIdentifiers(double)!!.toMutableMap()[identifier] = it.id
            }
            characterWithName?.id
        }
    }
    ...
    fun whenCharacterIsCreated(
        double: SoyleStoriesTestDouble,
        name: String = "New Character ${UUID.randomUUID()}"
    ): Character {
    ...
    fun getOpenCharacterListTool(double: SoyleStoriesTestDouble): CharacterList? {
        ...
        return findComponentsInScope<CharacterList>(projectScope).singleOrNull()
    ...
    fun isCharacterListToolShowingNumberOfCharacters(double: SoyleStoriesTestDouble, characterCount: Int): Boolean {
        ...
        interact {
            ...
            characterListSize =
                (projectScope.get<CharacterList>().root.lookup(".tree-view") as TreeView<*>).root.children.size
        }
    ...
    fun isCharacterListToolShowingCharacter(double: SoyleStoriesTestDouble, character: Character): Boolean {
        ...
        interact {
            ...
            characterItemViewModel =
                from(projectScope.get<CharacterList>().root).lookup(".tree-view").query<TreeView<*>>()
                    .root.children
                    .map { it.value }
    ...
    fun whenCharacterIsDeleted(double: SoyleStoriesTestDouble, characterId: Character.Id? = null): Character {
        ...
        recentlyDeletedCharacters =
            recentlyDeletedCharacters + (double to recentlyDeletedCharacters.getOrElse(double) { listOf() } + firstCharacter!!)
    ...
    fun whenCharacterListToolActionBarDeleteButtonIsClicked(double: SoyleStoriesTestDouble) {
        val projectScope = ProjectSteps.getProjectScope(double) ?: error("Project not yet created")
        interact {
            from(projectScope.get<CharacterList>().root).lookup("#actionBar_deleteLocation")
                .queryButton().onAction.handle(ActionEvent())
        }
    }
    ...
    fun isCharacterListToolShowingNameStoredForSelectedItem(double: SoyleStoriesTestDouble): Boolean {
        ...
        async(scope.applicationScope) {
            storedItem =
                scope.get<CharacterRepository>().getCharacterById(Character.Id(UUID.fromString(selectedItem.id)))
        }
